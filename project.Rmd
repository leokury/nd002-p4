...
ggplot(aes(x=fixed.acidity, y = volatile.acidity), data=wine_data) +  
  ...
  xlim(quantile(wine_data$fixed.acidity,0.001), quantile(wine_data$fixed.acidity,0.999)) + 
  ylim(quantile(wine_data$volatile.acidity,0.001), quantile(wine_data$volatile.acidity,0.999)) 
```
...
ggplot(aes(x=density, y = alcohol), data=wine_data) +  
  coord_cartesian(
    xlim=c(quantile(wine_data$density, 0.01), quantile(wine_data$density, 0.99)), 
    ylim=c(quantile(wine_data$alcohol, 0.01), quantile(wine_data$alcohol, 0.99))
    ) + 
  geom_jitter(alpha=1/2, aes(size=class, color=class)) +
  geom_vline(xintercept = mean(wine_data$density), linetype='longdash', color='blue', alpha=.5) +
  geom_hline(yintercept = mean(wine_data$alcohol), linetype='longdash', color='blue', alpha=.5)
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=10}
ggplot(aes(x=fixed.acidity, y =volatile.acidity, colour=class), data=wine_data) +  
  coord_cartesian(
    xlim=c(quantile(wine_data$fixed.acidity, 0.01), quantile(wine_data$fixed.acidity, 0.99)), 
    ylim=c(quantile(wine_data$volatile.acidity, 0.01), quantile(wine_data$volatile.acidity, 0.99))
    ) + 
  geom_jitter(alpha=1/2, aes(size=class, color=class)) +
  geom_vline(xintercept = mean(wine_data$fixed.acidity), linetype='longdash', color='blue', alpha=.5) +
  geom_hline(yintercept = mean(wine_data$volatile.acidity), linetype='longdash', color='blue', alpha=.5)
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=10}
ggplot(aes(x=free.sulfur.dioxide, y = total.sulfur.dioxide, colour=class), data=wine_data) +  
   coord_cartesian(
    xlim=c(quantile(wine_data$free.sulfur.dioxide, 0.01), quantile(wine_data$free.sulfur.dioxide, 0.99)), 
    ylim=c(quantile(wine_data$total.sulfur.dioxide, 0.01), quantile(wine_data$total.sulfur.dioxide, 0.99))
    ) + 
  geom_jitter(alpha=1/2, aes(size=class, color=class)) +
  geom_vline(xintercept = mean(wine_data$free.sulfur.dioxide), linetype='longdash', color='blue', alpha=.5) +
  geom_hline(yintercept = mean(wine_data$total.sulfur.dioxide), linetype='longdash', color='blue', alpha=.5)
```

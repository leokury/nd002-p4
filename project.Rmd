...
```{r echo=FALSE, message=FALSE, warning=FALSE}
ggplot(aes(x=density, y = alcohol, colour=class), data=wine_data) +  
  geom_point(alpha=1/2, position='jitter') +
  scale_colour_brewer(palette = "RdYlGn") + 
  xlim(min(wine_data$density), quantile(wine_data$density,0.99)) +
...
ggplot(aes(x=fixed.acidity, y =volatile.acidity, colour=class), data=wine_data) +  
  ...
  xlim(min(wine_data$fixed.acidity), quantile(wine_data$fixed.acidity,0.999)) +
  ylim(min(wine_data$volatile.acidity), quantile(wine_data$volatile.acidity,0.99))
```
